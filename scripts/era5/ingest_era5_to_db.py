...
ÉTAPE    : 7 — Ingestion des données ERA5 en DB PostgreSQL
FICHIER  : scripts/era5/ingest_era5_to_db.py
RÔLE     : Importe les CSV ERA5 dans la table era5_truth de PostgreSQL.
═══════════════════════════════════════════════════════════════════════════

STRATÉGIE D'INGESTION
---------------------
1. Crée une TABLE TEMPORAIRE identique à era5_truth
...
4. Supprime la table temporaire (automatique avec ON COMMIT DROP)
...
DIFFÉRENCES vs ingest_graphcast_to_db.py
-----------------------------------------
- Pas de run_timestamp (ERA5 = vérité terrain, pas une prévision)
- Pas de forecast_horizon_h
- Clé UNIQUE plus simple : (timestamp, variable_name, latitude, longitude)
...
-----
    ...
    # Ingérer TOUS les CSV
    python -m era5.ingest_era5_to_db

    # Ingérer UN fichier spécifique
    python -m era5.ingest_era5_to_db --csv era5_20260417.csv

    # Mode test (dry-run)
    python -m era5.ingest_era5_to_db --dry-run
...
CSV_DIR = BASE_DIR / "data" / "era5_csv"

TABLE_NAME = "era5_truth"
TEMP_TABLE_NAME = "era5_truth_staging"
...
def transform_csv_row(row: dict) -> tuple:
    ...
    CSV source :
        timestamp_utc, lat, lon, variable_name, value, unit

    DB cible :
        timestamp, variable_name, unit, latitude, longitude, value
    """
    return (
...
def csv_to_buffer(csv_path: Path) -> io.StringIO:
    """Convertit un CSV au format DB dans un buffer compatible psycopg2 COPY."""
    buffer = io.StringIO()
...
def ingest_csv_to_db(csv_path: Path, conn, dry_run: bool = False) -> dict:
    """
    Ingère un CSV ERA5 dans la DB via la stratégie temp table + UPSERT.

    Retourne un dict avec les stats : {rows_csv, rows_affected, duration}
    """
...
def validate_ingestion(conn):
    ...
    try:
        ...
        cursor.execute(f"SELECT COUNT(DISTINCT timestamp) FROM {TABLE_NAME}")
        n_timestamps = cursor.fetchone()[0]
        logger.info(f"   📊 Timestamps distincts : {n_timestamps}")
        ...
        logger.info(f"\n   📅 Plage temporelle :")
        logger.info(f"      • De {min_ts} à {max_ts}")
        ...
        # Stats tp_6h
        cursor.execute(f"""
            SELECT MIN(value), MAX(value), AVG(value), SUM(CASE WHEN value > 0 THEN 1 ELSE 0 END)
            FROM {TABLE_NAME}
            WHERE variable_name = 'tp_6h_mm'
        """)
        row = cursor.fetchone()
        if row[0] is not None:
            logger.info(f"   🌧️  tp_6h_mm : min={row[0]:.3f}mm, max={row[1]:.3f}mm, moy={row[2]:.3f}mm ({row[3]:,} valeurs > 0)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Ingestion CSV ERA5 → PostgreSQL",
    )
    parser.add_argument(
        "--csv", type=str, default=None,
        help="Nom d'un CSV spécifique (dans era5_csv/). Sinon tous.",
    )
    parser.add_argument(
    ...
    if args.csv:
        ...
        csv_paths = sorted(CSV_DIR.glob("era5_*.csv"))
    ...
    logger.info("\n" + "╔" + "═" * 68 + "╗")
    msg = f"✅ INGESTION TERMINÉE en {elapsed:.1f}s"
    logger.info(f"║  {msg:<66s}║")
    logger.info("╚" + "═" * 68 + "╝")
    logger.info(f"\n📊 Résultats :")
