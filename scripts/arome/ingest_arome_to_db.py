...
ÉTAPE    : 7 — Ingestion des prévisions AROME en DB PostgreSQL
FICHIER  : scripts/arome/ingest_arome_to_db.py
RÔLE     : Importe les CSV AROME dans la table arome_forecasts.
═══════════════════════════════════════════════════════════════════════════

STRATÉGIE D'INGESTION (identique à GraphCast)
----------------------------------------------
1. Crée une TABLE TEMPORAIRE identique à arome_forecasts
2. Utilise COPY pour charger TRÈS RAPIDEMENT le CSV
3. Fait un INSERT ... ON CONFLICT DO UPDATE depuis la temp vers la vraie table
4. Supprime la table temporaire (ON COMMIT DROP)
...
-----
    ...
    python -m arome.ingest_arome_to_db

    # Ingérer UN fichier spécifique
    python -m arome.ingest_arome_to_db --csv arome_2026-04-23_18z.csv

    # Mode test (dry-run)
    python -m arome.ingest_arome_to_db --dry-run

TEMPS ESTIMÉ : ~5-10 sec par CSV
═══════════════════════════════════════════════════════════════════════════
"""
...
CSV_DIR = BASE_DIR / "data" / "arome_csv"

TABLE_NAME = "arome_forecasts"
TEMP_TABLE_NAME = "arome_forecasts_staging"
...
# Colonnes mises à jour en cas de conflit
UPDATE_COLUMNS = ["unit", "value", "forecast_horizon_h"]
...
def ingest_csv_to_db(csv_path: Path, conn, dry_run: bool = False) -> dict:
    """Ingère un CSV AROME dans la DB via temp table + UPSERT."""
...
def validate_ingestion(conn):
    """Exécute des requêtes de validation après ingestion."""
    logger.info("\n" + "=" * 70)
...
def main():
    parser = argparse.ArgumentParser(description="Ingestion CSV AROME → PostgreSQL")
    parser.add_argument("--csv", type=str, default=None,
                        help="Nom d'un CSV spécifique. Sinon tous.")
    ...
    # Banner
    logger.info("╔" + "═" * 68 + "╗")
    msg = "📥 INGESTION AROME → PostgreSQL"
    ...
    if args.csv:
        ...
        csv_paths = sorted(CSV_DIR.glob("arome_*.csv"))
    ...
    if results["failed"]:
        ...
        for csv_path, err in results["failed"]:
            logger.warning(f"   • {csv_path.name} : {err[:100]}")
